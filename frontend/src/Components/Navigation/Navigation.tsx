...
const Header = () => {
    ...
    const toggleMenu = () => {
        ...
        setDropdownOpen(false); // 메뉴 토글 시 드롭다운 닫기
    };
    ...
    useEffect(() => {
        closeDropdown(); // 페이지 전환 시 드롭다운 닫기
    }, [location.pathname]);
            ...
            <div className="flex lg:flex-row items-center justify-between w-full lg:px-0" style={{ height: '80px' }}>
                ...
                <div className={`hidden lg:flex lg:items-center lg:ml-auto lg:w-auto space-x-8 px-4 ${location.pathname === "/" ? 'hidden' : ''}`}>
                    <ul className="flex space-x-4">
                        <li>
                            <Link to="/application" className={`block py-2 ${location.pathname === "/" ? 'hidden' : ''}`} onClick={closeDropdown}>등록하기</Link>
                        </li>
                        <li>
                            <Link to="/servicecenter" className={`block py-2 ${location.pathname === "/" ? 'hidden' : ''}`} onClick={closeDropdown}>고객센터</Link>
                        </li>
                    </ul>
                    <ul className="flex space-x-4">
                        {isLoggedIn ? (
